...
def eval_model(args):
    ...
    for line in tqdm(questions):
        ...
        # Track frame time and increment frame count
    ...
    # Finalize FPS and memory stats
    end_time = time.time()
    total_duration = end_time - start_time
    fps = total_frames / total_duration
    peak_memory_used = torch.cuda.max_memory_allocated(device="cuda:0") / (1024 ** 3)  # GB

    # Close answer file and print performance metrics
    ans_file.close()
    print(f"Total frames: {total_frames}")
    print(f"Total duration: {total_duration:.3f} seconds")
    print(f"FPS: {fps:.2f}")
    print(f"Peak GPU memory used: {peak_memory_used:.3f} GB")
