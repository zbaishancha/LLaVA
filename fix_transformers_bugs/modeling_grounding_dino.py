...
class GroundingDinoMultiscaleDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        if self.disable_custom_kernels:
            ...
            output = multi_scale_deformable_attention(value.float(), spatial_shapes, sampling_locations, attention_weights.float()).to(dtype=ori_dtype)
        else:
            ori_dtype = value.dtype
            try:
                # custom kernel
...
class GroundingDinoEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        ).to(dtype=vision_features.dtype)
...
class GroundingDinoDecoder(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            query_pos = get_sine_pos_embed(reference_points_input[:, :, 0, :], num_pos_feats=self.config.d_model // 2).to(dtype=dtype)
