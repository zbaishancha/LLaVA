            ...
            if isinstance(image_file, list):
                ...
                for img_file in image_file:
                    if image_folder is not None:
                        image.append(Image.open(os.path.join(
                            image_folder, img_file)).convert('RGB'))
                    else:
            ...
            if self.data_args.image_aspect_ratio == 'pad':
                ...
                if isinstance(image, list):
                    image = [expand2square(img, tuple(
                        int(x*255) for x in processor.image_mean)) for img in image]
                    image = [processor.preprocess(img, return_tensors='pt')[
                        'pixel_values'][0] for img in image]
                else:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            if isinstance(images[0], list):
                images = torch.stack([torch.stack(img, dim=0)
